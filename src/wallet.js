...
export class HDWalletManager {
  ...
  async getKeyPairByPathAndIndex(basePath, chain, index) {
  const hdWallet = await this.getHDWallet();
  const account = hdWallet.derivePath(basePath);
  const child = account.derive(chain).derive(index);
    
  const network = window.BLOCKCHAIN_CONFIG?.NETWORK || BLOCKCHAIN_CONFIG.NETWORK;
  
  if (!window.ECPair) {
    throw new Error(getTranslation('wallet.bitcoin_library_unavailable', 'Bitcoin library unavailable'));
  }

  const keyPair = window.ECPair.fromPrivateKey(child.privateKey, { network });
