...
export class WalletApp {
  ...
  // === PLACEHOLDER REPLACEMENT FOR TRANSLATIONS ===
  applyPlaceholdersRecursive(obj) {
    if (typeof obj === 'string') {
      // Utiliser replacePlaceholders depuis blockchain-config
      if (window.replacePlaceholders && typeof window.replacePlaceholders === 'function') {
        return window.replacePlaceholders(obj);
      }
      return obj;
    }
    ...
    if (typeof obj === 'object' && obj !== null) {
      ...
      for (const key in obj) {
        if (obj.hasOwnProperty(key)) {
          result[key] = this.applyPlaceholdersRecursive(obj[key]);
        }
      }
      return result;
    }
        ...
        window.i18next
          ...
          .init({
            ...
            backend: {
              ...
              // Parser personnalisé pour remplacer les placeholders
              parse: (data) => {
                const parsed = JSON.parse(data);
                return this.applyPlaceholdersRecursive(parsed);
              }
            },
